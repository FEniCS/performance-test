...
PYBIND11_MODULE(catalyst_pybind, m) {
  m.def("init", &CatalystAdaptor::Initialize, "Initialize");
  m.def("execute", &CatalystAdaptor::Execute, "Execute");
  m.def("finalize", &CatalystAdaptor::Finalize, "Finalize");
}
